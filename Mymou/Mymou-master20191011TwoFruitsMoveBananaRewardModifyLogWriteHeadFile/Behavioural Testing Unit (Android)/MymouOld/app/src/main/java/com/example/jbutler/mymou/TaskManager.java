...
//    change aa BananaReward WriteHeadFile this function commitTrialHeadData(String headMessage)

    public static void commitTrialHeadData(String headMessage){
        trialData.add(headMessage);
        if(trialData!=null)
        {
            String ss = trialData.get(0);
            logHandler.post(new LogEvent(ss));
            resetTrialData();
        }
...
//        String msg = TaskManager.photoTimestamp + "," + timestamp + "," + data; //change aa BananaReward
        String msg =  TaskExample.timestampStart + "," + timestamp + "," + data;
        trialData.add(msg);
    }
    //重置实验数据；
