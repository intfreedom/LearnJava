...
public class TaskExample extends Fragment
    ...
    public static String HeadMessage = "\n%HeadFile\nTask: 001 is TaskID, Apple and Banana is coexist, Click on 'Banana' to get rewards，Button display time is 1000ms，" +
            "The button disappears time is 2000ms.Picture banana has a pixel size of 100×70, picture apple has a pixel size of 100×114\n" +"Date: "+timestampToday+
    ...
    * Bundle用来存储恢复数据，可供该方法从保存状态下重建视图；*/
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        TaskManager.commitTrialHeadData(HeadMessage);
...
//        TaskManager.deliverReward(juiceChoice,rewardAmount);  //change aa BananaReward
        endOfTrial(0,rewardAmount + 500);//暂时取消，不知道会有何影响  change aa
    }
...
//        PrepareForNewTrial(newTrialDelay);//onePicture
    ...
    private static void randomiseNoReplacement(Button[] buttons) {
        ...
        int choice = r.nextInt(maxCueLocations);//change aa BananaReward
        while(choice==2||choice==5){
            choice = r.nextInt(maxCueLocations);
        }
        for (int i = 0; i < buttons.length; i++) {
            while (chosen[choice] == 1||choice==2||choice==5) {//保证两个按钮的位置不重叠；//change aa BananaReward
                choice = r.nextInt(maxCueLocations);
            }
...
//                randomiseNoReplacement(cues_O);//changetask-onePicture //changeDon'tMove note
        ...
        }, 1500);//changetask  changeReward  change aa change aa BananaReward small
    }
